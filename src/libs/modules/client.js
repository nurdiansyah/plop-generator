...
export const moduleClientGenerator = ({ plop, prompts }) => ({ actions = [], templateDir, path, data = {} }) => {
    const model = pascalCase(data.model), modulePackage = "client", rootPath = `${path}${data.isMonorepo ? `/${modulePackage}` : ""}`, srcPath = `${rootPath}/src`;
    ...
    const graphqlActions = [
        ...
        getTemplateExportIndexAction(`./${model}Document`, `${srcPath}/graphql/index.ts`),
    ];
    const serviceActions = [
        ...
        getTemplateExportIndexAction(`./${model}Service`, `${srcPath}/services/index.ts`),
    ];
    const svelteStoresActions = [
        {
            type: "add",
        ...
        getTemplateExportIndexAction(`./${model}Store`, `${srcPath}/stores/index.ts`),
    ];
