...
export const templateFilesGenerator = ({ plop, env = {}, skipPattern = /.*(\.custom)$/, recursive = true, path, prompts, actions = [], templateDir, }) => {
    recursivePrompts({
        templateDir,
        pathDir: templateDir || "",
        prompts,
    ...
    return (options) => recursiveFilesAction({
        plop,
        ...
        if (fs.existsSync(path) && fs.lstatSync(path).isDirectory()) {
            ...
            if (choices.length) {
                const prompt = {
                    ...
                    choices,
                    when: (answers) => answers.workspace ===
...
function recursiveFilesAction({ plop, prompts, templateDir = "templates", skipPattern, path, data, actions, recursive, }) {
